    ...
    publish(content, routingKey = '', options = {}) {
        ...
        this._initialized.then(() => {
            try {
                ...
                connection_factory_rabbitmq_1.log.log('warn', 'Exchange publish error: ' + err.messageBus, { module: 'amqp-ts' });
        ...
        const processRpc = () => {
            ...
            if (Object.keys(this._channel.consumers).length === 0) {
                this._channel.consume(DIRECT_REPLY_TO_QUEUE, (resultMsg) => {
                    const result = new bus_message_1.BusMessage(resultMsg.content, resultMsg.properties);
            ...
            const message = new bus_message_1.BusMessage(requestParameters, { correlationId: uuid, replyTo: DIRECT_REPLY_TO_QUEUE });
            message.sendTo(this, routingKey);
        };
