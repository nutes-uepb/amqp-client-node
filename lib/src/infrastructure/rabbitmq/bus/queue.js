    ...
    publish(content, options = {}) {
        ...
        const sendMessage = () => {
            try {
                ...
                connection_factory_rabbitmq_1.log.log('debug', 'Queue publish error: ' + err.message, { module: 'amqp-ts' });
        ...
        return new Promise((resolve, reject) => {
            const processRpc = () => {
                ...
                this._channel.consume(DIRECT_REPLY_TO_QUEUE, (resultMsg) => {
                    ...
                    const result = new message_1.Message(resultMsg.content, resultMsg.fields);
                    ...
                    if (err) {
                        ...
                        const message = new message_1.Message(requestParameters, { replyTo: DIRECT_REPLY_TO_QUEUE });
                        message.sendTo(this);
                    }
                });
            };
    ...
    _initializeConsumer() {
        const processedMsgConsumer = (msg) => {
            try {
                ...
                if (msg.properties.replyTo) {
                    ...
                    if (result instanceof Promise) {
                        result.then((resultValue) => {
                            ...
                            connection_factory_rabbitmq_1.log.log('error', 'Queue.onMessage RPC promise returned error: ' + err.message, { module: 'amqp-ts' });
                        });
                    }
                    else {
                ...
                connection_factory_rabbitmq_1.log.log('error', 'Queue.onMessage consumer function returned error: ' + err.message, { module: 'amqp-ts' });
            }
        };
                    ...
                    if (result instanceof Promise) {
                        result.then((resultValue) => {
                            if (!(resultValue instanceof message_1.Message)) {
                                resultValue = new message_1.Message(resultValue, {});
                            }
                        ...
                        if (!(result instanceof message_1.Message)) {
                            result = new message_1.Message(result, {});
                        }
