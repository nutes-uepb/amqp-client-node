...
class ServerRegisterRabbitmq {
    constructor(_connection, _queueName, _exchangeName, _routingKeys, _options) {
        this._connection = _connection;
        this._queueName = _queueName;
        this._exchangeName = _exchangeName;
        this._routingKeys = _routingKeys;
    ...
    registerRoutingKeyServer(queueName, exchangeName, routingKey, options) {
        return new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const exchange = yield this._connection.getExchange(exchangeName, options ? options.exchange : undefined);
                ...
                if (routingKey.length > 0)
                    for (const key of routingKey)
                        yield queue.bind(exchange, key);
                else
                    for (const value of this.resource_handlers.get(queueName))
                        yield queue.bind(exchange, value.resource_name);
                yield this.routingKeyServerConsumer(queue, options.consumer);
            }
            catch (err) {
                return reject(err);
            }
    ...
    routingKeyServerConsumer(queue, consumer) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!queue.consumerInitialized) {
                try {
                    yield queue.activateConsumer((message) => {
                        const clientRequest = message.content;
                        const resources_handler = this.resource_handlers.get(queue.name);
                        if (resources_handler) {
                    ...
                    }, Object.assign({}, consumer, { noAck: true }));
