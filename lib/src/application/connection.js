...
class Connection {
    ...
    sub(queueName, exchangeName, routingKey, callback, options) {
        ...
        return this._sub.receiveRoutingKeyMessage(queueName, exchangeName, routingKey, eventCallback, options);
    }
    createRpcServer(queueName, exchangeName, routingKeys, options) {
        let server = this._rpcServers.get(queueName);
        if (!server) {
            server = new server_register_rabbitmq_1.ServerRegisterRabbitmq(this._eventBusConnection, queueName, exchangeName, routingKeys, options);
